-- Shareable parent payment links for Kids Sadaqah (Stripe)
...
create index if not exists donation_payment_requests_token_idx
  on public.donation_payment_requests (token);
...
drop policy if exists "Users can view own donation payment requests" on public.donation_payment_requests;
create policy "Users can view own donation payment requests"
  on public.donation_payment_requests for select
  to authenticated
  using (auth.uid() = user_id);

drop policy if exists "Users can create own donation payment requests" on public.donation_payment_requests;
create policy "Users can create own donation payment requests"
  on public.donation_payment_requests for insert
  to authenticated
...
grant select, insert on public.donation_payment_requests to authenticated;
