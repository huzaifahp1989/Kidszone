...
const currencyAccent = 'from-[#115e59] via-[#0f766e] to-[#115e59]';
...
export function VoucherHub({ initialView = 'offers' }: VoucherHubProps) {
  ...
  const statCards = [
    ...
    { label: 'Your live vouchers', value: activeHistory.length, tone: 'bg-[#f0fdfa] text-[#166534]' },
    ...
    <section className="space-y-6">
      <div className="relative overflow-hidden rounded-[2rem] border border-[#cbd5e1] bg-[radial-gradient(circle_at_top_left,_rgba(236,253,245,0.95),_rgba(255,255,255,0.95)_45%,_rgba(240,249,255,0.95))] p-6 shadow-[0_32px_100px_rgba(15,118,110,0.16)] dark:border-slate-700 dark:bg-slate-900">
        ...
        <div className="relative z-10 grid gap-6 md:grid-cols-[1.7fr_1fr] md:items-end">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full bg-white/85 px-3 py-1 text-xs font-black uppercase tracking-[0.24em] text-[#115e59] shadow-sm">
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              <button
                ...
                className="rounded-2xl bg-[#115e59] px-5 py-3 text-sm font-black text-white shadow-lg shadow-emerald-900/15 transition hover:bg-[#115e59]"
              >
                Browse offers
              </button>
              <Link
                href="/my-vouchers"
                className="rounded-2xl border border-[#99f6e4] bg-white px-5 py-3 text-sm font-black text-[#115e59] transition hover:bg-[#f0fdfa]"
              >
                Open My Vouchers
              </Link>
            </div>
      ...
      {actionMessage && <div className="rounded-2xl border border-[#99f6e4] bg-[#f0fdfa] px-4 py-3 text-sm font-semibold text-[#115e59]">{actionMessage}</div>}

      {notifications.length > 0 && (
        <div className="rounded-[1.8rem] border border-[#e2e8f0] bg-white/90 p-5 shadow-sm">
          <div className="flex items-center gap-2 text-slate-900">
            <BellRing size={18} className="text-[#115e59]" />
            <h3 className="text-lg font-black">Reward updates</h3>
          </div>
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            {notifications.slice(0, 3).map((item) => (
              <div key={item.id} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                <p className="text-xs font-black uppercase tracking-[0.18em] text-[#115e59]">{item.type.replace(/_/g, ' ')}</p>
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-black uppercase tracking-[0.18em] text-[#115e59]">{offer.discountLabel || offer.discountType.replace('_', ' ')}</p>
                      ...
                      >
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            ...
                            <p className="mt-3 text-xs font-black uppercase tracking-[0.18em] text-[#115e59]">{item.voucherCode}</p>
                          </div>
          ...
          <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
            {gallery.map((item) => (
              <article key={item.id} className="overflow-hidden rounded-[1.8rem] border border-slate-200 bg-white shadow-sm">
                ...
                <div className="p-5">
                  <p className="text-xs font-black uppercase tracking-[0.18em] text-[#115e59]">{item.businessName}</p>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-[1.8rem] border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-2 text-slate-900"><Clock3 size={18} className="text-[#115e59]" /><h4 className="font-black">24-hour live window</h4></div>
        ...
        <div className="rounded-[1.8rem] border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-2 text-slate-900"><QrCode size={18} className="text-[#115e59]" /><h4 className="font-black">QR-ready checkout</h4></div>
        ...
        <div className="rounded-[1.8rem] border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-2 text-slate-900"><ShieldCheck size={18} className="text-[#115e59]" /><h4 className="font-black">Anti-fraud checks</h4></div>
