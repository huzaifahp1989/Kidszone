...
export async function runPointsHealthCheck(): Promise<PointsHealthReport> {
  ...
  if (!checks.supabase_url_configured) {
    issues.push({
      ...
      fixHint:
        'Set NEXT_PUBLIC_SUPABASE_URL (and anon/service_role keys) on the live Vercel project (huzaifahp1989-audio.vercel.app). Keep LIVE_APP_URL / Capacitor pointed at that host once env is set.',
    });
