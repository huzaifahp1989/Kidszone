...
- **Env file:** Copy `.env.example` to `.env.local` (gitignored). Do NOT keep the literal placeholder values `your_supabase_project_url` / `your_supabase_anon_key` from `.env.example` — an invalid Supabase URL string makes `createClient` throw. Instead leave `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY` blank so local `src/lib/supabase.ts` falls back to the safe `https://placeholder.supabase.co` client. With this fallback the dev server runs fine (Supabase calls just fail gracefully); the console logs `[supabase] ... missing` warnings, which are expected. On Vercel/`NODE_ENV=production`, the client falls back to the known production project instead of placeholder so missing env cannot break quiz submit/points.
- **Live Cap URL:** `capacitor.config.ts` / `src/lib/app-url.ts` must point at a deploy with real Supabase env (`islamic-kids-platform.vercel.app`). `huzaifahp1989-audio.vercel.app` previously shipped `placeholder.supabase.co` and broke answer submit + points; middleware 307s that host to the working project until Cap apps are rebuilt.
