...
`ONESIGNAL_REST_API_KEY` for Production / Preview / Development on the `islamic-kids-platform` Vercel project was updated to the App API Key for OneSignal app `0bb81263…`, then Production was redeployed. Admin → Push → Diagnose should now show REST auth OK for the primary app (delivery still depends on FCM/APNs and valid device subscriptions).
